...
function ok(schema: { safeParse: (v: unknown) => { success: boolean } }, value: unknown, label = '') {
  ...
  assert.equal(result.success, true, `Expected valid${label ? ` (${label})` : ''}: ${JSON.stringify(value)}`);
}

function fail(schema: { safeParse: (v: unknown) => { success: boolean } }, value: unknown, label = '') {
  const result = schema.safeParse(value);
  assert.equal(result.success, false, `Expected invalid${label ? ` (${label})` : ''}: ${JSON.stringify(value)}`);
}
...
describe('RubricJsonSchema', () => {
  ...
  it('accepts a rubric with optimization budget', () =>
    ok(RubricJsonSchema, { ...minimalRubric, optimization: { maxNodes: 6, maxEdges: 6 } }));

  it('accepts a rubric with a built-in anti-pattern ref', () =>
    ok(RubricJsonSchema, {
      ...minimalRubric,
      antiPatterns: [{ ref: 'unencrypted-cross-zone' }],
    }));

  it('accepts a rubric with an inline anti-pattern', () =>
    ok(RubricJsonSchema, {
      ...minimalRubric,
      antiPatterns: [
        {
  ...
  it('rejects optimization with non-integer maxNodes', () =>
    fail(RubricJsonSchema, { ...minimalRubric, optimization: { maxNodes: 6.5, maxEdges: 6 } }));

  it('rejects optimization with zero maxNodes', () =>
    fail(RubricJsonSchema, { ...minimalRubric, optimization: { maxNodes: 0, maxEdges: 6 } }));
...
describe('PuzzleJsonSchema — SPA puzzle', () => {
  const spaPuzzle = {
    ...
    rubric: {
      requirements: [
        {
          ...
          label: 'Visitors should find the signboard by a name they can remember.',
          hint: 'Use the DNS to give internet traffic a URL to visit.',
        },
        {
          id: 'storage-not-public',
          label: 'Keep the scrolls inside the castle.',
          hint: 'Object Storage should not be directly reachable from the internet.',
        },
        {
          id: 'waf-bonus',
          label: 'Bonus WAF.',
      ...
      datalogRules: 'req("starts-from-internet", "pass") :- node(I, "internet"), node(D, "dns"), edge(I, D).',
    },
  };
  ...
  it('rejects SPA puzzle with empty requirements', () => {
    fail(PuzzleJsonSchema, { ...spaPuzzle, rubric: { ...spaPuzzle.rubric, requirements: [] } });
  });
});
...
describe('PuzzleJsonSchema — Midnight Market', () => {
  const midnightMarket = {
    ...
    initialNodes: [
      { id: 'preplaced-internet', nodeType: 'internet', position: { x: -320, y: 200 }, deletable: false },
      { id: 'preplaced-payment-processor', nodeType: 'payment_processor', position: { x: 560, y: 400 }, deletable: false },
    ],
    rubric: {
      requirements: [
        {
  ...
  it('validates the Midnight Market fixture', () => ok(PuzzleJsonSchema, midnightMarket));

  it('rejects a node in initialNodes with an invalid position', () =>
    fail(PuzzleJsonSchema, {
      ...midnightMarket,
      initialNodes: [{ id: 'bad', nodeType: 'internet', position: 'not-an-object' }],
    }));
});
