  ...
  rubric: {
    requirements: [
      ...
      },
      {
        ...
        label:
          'A protective ward must filter the mob before it reaches the gate.',
        hint: 'Apply a WAF to the API Gateway to filter malicious traffic at the boundary.',
      },
      {
        id: 'lb-distributes-load',
        label:
          'The midnight mob must be spread across many workers, lest any one scribe collapse.',
        hint: 'Place a Load Balancer between the API Gateway and your compute workers.',
      },
      {
        id: 'catalog-on-compute',
        label:
          'The Catalog Service cannot cast its spell in mid-air — it must be bound to a servant.',
      ...
      },
      {
        ...
        label:
          'The keys to the Royal Mint must be kept in a sealed vault, never left in the open.',
      ...
      },
      {
        ...
        label:
          'The payment scribe must be able to reach the Royal Mint to collect the coin-seals.',
        hint: "Connect the Payment Service's compute to the external Payment Processor.",
      },
      {
        id: 'cdn-reduces-origin-load',
        label:
          'Let distant copies of the wares reach browsing hands without taxing the central storehouse. (Bonus)',
      ...
      },
      {
        ...
        label:
          "The Guild must be able to watch the Market's health when the midnight rush arrives. (Bonus)",
    ...
    restrictions: [
      {
        ...
        label: 'Coin-seals must never flow directly from the shopfront to the Payment Alcove.',
        hint: 'The Payment Processor must only be reachable after passing through a Message Queue — no direct path from ingress to payment.',
        detect: {
          op: 'canReach',
          from: ['internet', 'cdn', 'load_balancer', 'api_gateway'],
          to: ['payment_processor'],
          skip: ['queue', 'event_bus', 'streaming'],
          message: 'A direct path exists from your ingress to the Payment Processor without passing through a Message Queue — payment must be decoupled asynchronously.',
        },
      },
    ],
